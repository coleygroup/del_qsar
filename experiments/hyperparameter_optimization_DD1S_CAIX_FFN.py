...
def objective(trial):
    ...
    INIT_LR = trial.suggest_float('init_lr', low=1e-5, high=1e-1, log=True)
    ...
    if NUM_LAYERS > 1: 
        LAYERS_TYPE = trial.suggest_categorical('layer_size_architecture', ['flat', 'pyramid_2', 'pyramid_4'])
    MAX_LAYER_SIZE_EXP = trial.suggest_int('max_layer_size_exponent', low=4, high=10, step=1) # largest layer size: 16 to 1024                               
    ...
    if NUM_LAYERS == 1:
        LAYER_SIZES = [2**MAX_LAYER_SIZE_EXP]
    elif LAYERS_TYPE == 'flat':
        LAYER_SIZES = [2**MAX_LAYER_SIZE_EXP for _ in range(NUM_LAYERS)]
    elif LAYERS_TYPE == 'pyramid_2': # layer size decreases by factor of 2
        LAYER_SIZES = []
        exp = MAX_LAYER_SIZE_EXP
        for _ in range(NUM_LAYERS):
            LAYER_SIZES.append(2**exp) 
            exp -= 1
    elif LAYERS_TYPE == 'pyramid_4': # layer size decreases by factor of 4
        LAYER_SIZES = []
        exp = MAX_LAYER_SIZE_EXP
        for _ in range(NUM_LAYERS):
            LAYER_SIZES.append(2**exp)
        ...
        raise ValueError('Unknown layer size architecture')
