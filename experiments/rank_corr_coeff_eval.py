...
def main(argv):
    ...
    if 'DD1S_CAIX' in FLAGS.csv:
        RESULTS_FILE = os.path.join(SAVE_ROOT, f'DD1S_CAIX_{FLAGS.model_type}_rank_corr_coeff_eval.csv')
    elif 'sEH' in FLAGS.exp[0]:
        RESULTS_FILE = os.path.join(SAVE_ROOT, f'triazine_sEH_{FLAGS.model_type}_rank_corr_coeff_eval.csv')
    elif 'SIRT2' in FLAGS.exp[0]:
        RESULTS_FILE = os.path.join(SAVE_ROOT, f'triazine_SIRT2_{FLAGS.model_type}_rank_corr_coeff_eval.csv')
    ...
    for i in range(len(all_splitters)):
        ...
        for seed in range(num_replicates):
            ...
            if 'D-MPNN' in FLAGS.model_type and 'triazine' in FLAGS.csv: # triazine D-MPNN
                ...
                if 'sEH' in FLAGS.exp[0] and len(FLAGS.exp) == 1:
                    ...
                    model.load_state_dict(torch.load(os.path.join(DELQSAR_ROOT, 'experiments',
                                                                  ...
                                                                  f'{splitter_names[i]}_seed_{seed}.torch')))
                elif len(FLAGS.exp) == 2:
                    model.load_state_dict(torch.load(os.path.join(DELQSAR_ROOT, 'experiments',
                                                                  f'models/triazine_sEH_SIRT2_multi-task/{FLAGS.model_type}', 
                                                                  f'{splitter_names[i]}_seed_{seed}.torch')))
            ...
            if 'D-MPNN' not in FLAGS.model_type:
                test_preds = model.predict_on_x(
                    x[test_slice, :], device='cuda:0',
                )
            else:
                test_preds = model.predict_on_x(
                    [x[i] for i in test_slice], device='cuda:0',
                )
            ...
            rank_corr_coeffs.append(np.squeeze(rank_corr_coeff))
            ...
            if not os.path.isfile(RESULTS_FILE):
                with open(RESULTS_FILE, 'w') as fid:
                    ...
                    cols.append('Rank correlation coefficient')
                    fid.write('\t'.join(cols) + '\n')
            with open(RESULTS_FILE, 'a') as fid:
                cols = [str(datetime.now().now())]
                cols.append(str(splitter_names[i]))
                cols.append(str(seed))
                cols.append(str(np.squeeze(rank_corr_coeff)))
                fid.write('\t'.join(cols) + '\n')
            fid.close()
        
        # average across replicates
        avg_rank_corr_coeff = sum(rank_corr_coeffs) / len(rank_corr_coeffs)
        formatted_avg_rank_corr_coeff = '{0:.5f}'.format(avg_rank_corr_coeff)
            
        logging.info(f'Average rank correlation coefficient: {np.squeeze(formatted_avg_rank_corr_coeff)}\n')                     
        
        with open(LOG_FILE, 'a') as lf: 
            lf.write(f'{datetime.now()} INFO: Average rank correlation coefficient: {np.squeeze(formatted_avg_rank_corr_coeff)}\n')
